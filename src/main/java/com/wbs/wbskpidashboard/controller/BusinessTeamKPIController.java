...
public class BusinessTeamKPIController {
        ...
        public List<BusinessTeamKPI> getMethodName(@PathVariable Long teamId) {
                ...
                if (businessTeam.isPresent()) {
                        ...
                        return businessTeamKPIs;

        }

        @GetMapping("wbs-kpi-dashboard/team/{teamId}/KPI/{kpiMeasure}")
        public List<BusinessTeamKPI> getTeamKpis(@PathVariable Long teamId, @PathVariable String kpiMeasure) {

                List<BusinessTeamKPI> kpiList = new ArrayList<>();
                Optional<BusinessTeam> businessTeam = businessTeamRepository.findById(teamId);

                if (businessTeam.isPresent()) {
                        return businessTeamKPIRepository.findByKpiBusinessMeasureAndTeamName(kpiMeasure,
                                        businessTeam.get().getTeamName());
                } else
                        return kpiList;
        ...
        public Map<String, TeamSummary> getMethodName() {
                ...
                for (BusinessTeam businessTeam : businessTeams) {
                        ...
                        Double avgFactoryUtilization = businessTeamKPIRepository.retreiveValue(
                                        businessTeam.getTeamName(),
                                        factoryUtilization);
                        Double avgEmployeeEngagement = businessTeamKPIRepository.retreiveValue(
                                        businessTeam.getTeamName(),
                                        employeeEngagement);
                        Double avgInterestCoverage = businessTeamKPIRepository.retreiveValue(businessTeam.getTeamName(),
                                        interestCoverage);
                        Double avgMarketingSpendRev = businessTeamKPIRepository.retreiveValue(
                                        businessTeam.getTeamName(),
                                        marketingSpendRev);
                        Double avgECarSales = businessTeamKPIRepository.retreiveValue(businessTeam.getTeamName(),
                                        eCarSales);
                        Double avgCo2Penalty = businessTeamKPIRepository.retreiveValue(businessTeam.getTeamName(),
                                        co2Penalty);
                        Double avgWacc = businessTeamKPIRepository.retreiveValue(businessTeam.getTeamName(), wacc);

                        TeamSummary teamSummary = new TeamSummary(avgWacc, avgScores, avgFactoryUtilization,
                                        avgEmployeeEngagement,
